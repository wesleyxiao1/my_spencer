            ...
            if len(line_data) == 2:
                ...
                if key in param_keys:
                    try:
                        ...
                        raise AssertionError("Incorrect value type in params.txt: " + line)

                    param_keys.remove(key)
                    params[key] = val
            else:
                #raise AssertionError(f"Error in params.txt formatting: {line}")
                raise AssertionError("Error in params.txt formatting: " + line)
...
def read_cli_parameters():
    ...
    logging.info("Input file is: " + input_file)
    logging.info("Output file is: " + output_file)
    logging.info("Parameter file is: " + param_file)

    # Read MHT parameters
    try:
        params = read_parameters(param_file)
        #logging.info(f"MHT parameters: {params}")
        logging.info("MHT parameters: " + str(params))
